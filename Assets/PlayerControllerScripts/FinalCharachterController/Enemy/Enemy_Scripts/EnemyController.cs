    ...
    public class EnemyController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(_distance > _detectionRange) ChangeState(EnemyState.Idle);

        else if(_distance <= _detectionRange) ChangeState(EnemyState.Chase);

        else{
            if(attackTimer <= 0f && _currentState != EnemyState.Attack)
            {
    ...
    private void AttackState(){ //3
        FacePlayer();
        _agent.isStopped=true;
        if(attackTimer<=0) attackTimer=_attackCooldown;
    }

    private void DeadState(){ //4
        _isDead = true;
